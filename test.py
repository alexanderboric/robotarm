...
segments = [
    ...
    segment("E", 10, [0, 1, 0], 0, 360),  # rotatin along the z axis endpiece
    segment("F", 10+attachment_length, (0, 1, 0), 0, 360,
            [0, 0, 1]),  # rotatin along the y axis endpiece
]
...
for i in range(len(segments)):
    ...
    # reading the current angle of the segment from json file and setting it to the current angle
    try:
        segments[i].currentangle = json.load(open("log.json"))[
            segments[i].name]
    except:
...
def calc_positions(segments):
    ...
    for i in range(len(segments)):
        ...
        currentPosition = (currentPosition[0]+segments[i].length*np.sin(segments[i].currentangle),
                           currentPosition[1]+segments[i].length*np.cos(segments[i].currentangle), currentPosition[2])
...
def calc_direction(segments):
    ...
    for i in range(len(segments)):
        currentDirection = (currentDirection[0]+segments[i].length*np.sin(segments[i].currentangle),
                            currentDirection[1]+segments[i].length*np.cos(segments[i].currentangle), currentDirection[2])
...
# current direction of the endpiece
currentDirection = calc_direction(segments)
...
def main():
    ...
    while running:
        ...
        pygame.draw.line(surf, (20, 20, 20), (int(origin_points[0][3]), int(
            origin_points[0][4])), (int(origin_points[1][3]), int(origin_points[1][4])), 2)
        pygame.draw.line(surf, (20, 20, 20), (int(origin_points[0][3]), int(
            origin_points[0][4])), (int(origin_points[2][3]), int(origin_points[2][4])), 2)
        pygame.draw.line(surf, (20, 20, 20), (int(origin_points[0][3]), int(
        ...
        for index in np.argsort(z_order):
            if z_order[index] == 9999:
                break
            triangle = [points[triangles[index][0]][3:],
                        points[triangles[index][1]][3:], points[triangles[index][2]][3:]]
            color = shade[index] * \
        ...
        pygame.display.set_caption(
            str(round(1/(elapsed_time+1e-16), 1)) + ' ' + str(camera))
...
def project_points(points, camera):
    ...
    for point in points:
        ...
        h_angle_camera_point = np.arctan(
            (point[2]-camera[2])/(point[0]-camera[0] + 1e-16))
        ...
        distance = np.sqrt(
            (point[0]-camera[0])**2 + (point[1]-camera[1])**2 + (point[2]-camera[2])**2)
...
def sort_triangles(points, triangles, camera, light_dir, z_order, shade):
    for i in range(len(triangles)):
        ...
        normal = normal / \
            np.sqrt(normal[0]*normal[0] + normal[1]
                    * normal[1] + normal[2]*normal[2])

        CameraRay = points[triangle[0]][:3] - camera[:3]
        dist2cam = np.sqrt(
            CameraRay[0]*CameraRay[0] + CameraRay[1]*CameraRay[1] + CameraRay[2]*CameraRay[2])
        CameraRay = CameraRay/dist2cam

        # get projected 2d points for filtering of offscreen triangles
        xxs = np.asarray([points[triangle[0]][3:5][0],
                         points[triangle[1]][3:5][0],  points[triangle[2]][3:5][0]])
        yys = np.asarray([points[triangle[0]][3:5][1],
                         points[triangle[1]][3:5][1],  points[triangle[2]][3:5][1]])
...
def movement(camera, elapsed_time):
    ...
    if pygame.mouse.get_focused():
        ...
        camera[3] = (camera[3] + 10*elapsed_time *
                     np.clip((p_mouse[0]-SCREEN_W/2)/SCREEN_W, -0.2, .2)) % (2*np.pi)
        camera[4] = camera[4] + 10*elapsed_time * \
            np.clip((p_mouse[1]-SCREEN_H/2)/SCREEN_H, -0.2, .2)
    ...
    for line in f:
        if line[:2] == "v ":
            ...
            vertex = [float(line[index1:index2]), float(
                line[index2:index3]), float(line[index3:-1]), 1, 1]
            ...
            triangles.append([int(line[index1:index2]) - 1,
                             int(line[index2:index3]) - 1, int(line[index3:-1]) - 1])
